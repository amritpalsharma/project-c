...
class UserController extends ResourceController
{
    ...
    public function updateProfile($id = null)
    {
        ...
        if (!empty(auth()->id()) && is_numeric(auth()->id()) && auth()->id() == 1 && empty($id)) {
            $activity_for = auth()->id();
            $activity_key = 'userUpdatedProfileDetailAdmin';
        } else {
            $activity_for = auth()->id();
            $activity_key = 'userUpdatedProfileDetail';
        }
        if (auth()->id() == 1 && !empty($id)) {
            $activity_key = 'userUpdatedProfileDetailAdmin';
        }
        $user_id = $id ?? auth()->id();
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP) || hasAccess(auth()->user(), EDITOR_ACCESS)) {
            if ($this->request->getVar('user') && count($this->request->getVar('user')) > 0) {
                ...
                // $contract_start = $this->request->getVar('user')['contract_start'] ?? '';
                // $contract_end = $this->request->getVar('user')['contract_end'] ?? '';
                // echo ' >>>>>>>>>>>>> contract_start >>>>> ' . $contract_start;
                // echo ' >>>>>>>>>>>>> contract_end    >>>>> ' . $contract_end;
                ...
                foreach ($this->request->getVar('user') as $key => $value) {
                    ...
                    if ($key == 'current_team') {
                        ...
                        if (strtolower($value) != "null" && $value > 0) {
                            ...
                            if (!$isExist) {
                                ...
                                if($this->request->getVar('user')['contract_start'] != 'Invalid date'){
                                    $club_data['join_date'] = $this->request->getVar('user')['contract_start'];
                                }

                                if($this->request->getVar('user')['contract_end'] != 'Invalid date'){
                                    $club_data['end_date'] = $this->request->getVar('user')['contract_end'];
                                }
                                ...
                                if ($isExist['team_id'] != $value) {
                                    ...
                                    if($this->request->getVar('user')['contract_start'] != 'Invalid date'){
                                        $in_data['join_date'] = $this->request->getVar('user')['contract_start'];
                                    }
    
                                    if($this->request->getVar('user')['contract_end'] != 'Invalid date'){
                                        $in_data['end_date'] = $this->request->getVar('user')['contract_end'];
                                    }
                        ...
                        if ( $oldClubID && $oldClubID != $value) {
                            ...
                            if ($getEmailTemplate) {
                                ...
                                $replacements = [
                                    ...
                                    '{faqLink}'         => '<a href="' . getPageInUserLanguage($oldClubInfo->user_domain, $oldClubInfo->lang, "faq") . '">' . lang('Email.faq', [], getLanguageCode($oldClubInfo->lang, true)) . '</a>',
                                    '{helpEmail}'       => '<a href="mailto:' . HELP_EMAIL . '">' . lang('Email.helpEmail', [], getLanguageCode($oldClubInfo->lang, true))  . '</a>',
                                ];
                                ...
                                $message = view('emailTemplateView', ['message' => $content, 'disclaimerText' => getEmailDisclaimer($oldClubInfo->lang), 'footer' => getEmailFooter($oldClubInfo->lang, $oldClubInfo->user_domain)]);
                    ...
                    if ($userMetaObject->save($save_data)) {
                        $response = [
                            "status"    => true,
                            "message"   => lang('App.profile_updatedSuccess', [], getLanguageCode($currentLang, true)),
                            "data"      => []
                        ];
                    } else {
                        $response = [
                            "status"    => false,
                            "message"   => lang('App.profile_updatedfailure', [], getLanguageCode($currentLang, true)),
                            "data"      => []
                        ];
                    }
                }
                ...
                $activityEvent = $activity_key;
                // echo $activityEvent; die;
                $replacements = [
                    '{USER_ID}' => '{USER_ID_' . $user_id . '}',
                ];
                $additionalData = ['activity_type_id' => 2, 'user_id' => $user_id]; // created
    ...
    public function updateGeneralInfo()
    {
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP)) {
            ...
            if ($res) {
                ...
                // create Activity log
                $activityEvent = 'userUpdatedGeneralInfo';
                $replacements = [
                    // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                    '{USER_ID}' => '{USER_ID_' . auth()->id() . '}',
                ];
                $additionalData = ['activity_type_id' => 2]; // created
                logActivity($activityEvent, $replacements, $additionalData);
                $response = [
                    "status"    => true,
                ...
                foreach ($id as $userId) {
                    ...
                    if ($user) {
                        ...
                        if ($getEmailTemplate) {
                            ...
                            $language->setLocale(getLanguageCode($user->lang, true));

                            $replacements = [
                                '{firstName}'   => $user->first_name,
                                '{faqLink}'     => '<a href="' . getPageInUserLanguage($user->user_domain, $user->lang, "faq") . '">' . lang('Email.faq') . '</a>',
                            ];
                            ...
                            // Replace placeholders with actual values in the email body
                            $content = strtr($getEmailTemplate['content'], $replacements);
                            $message = view('emailTemplateView', ['message' => $content, 'disclaimerText' => getEmailDisclaimer($user->lang), 'footer' => getEmailFooter($user->lang, $user->user_domain)]);
                        ...
                        $getAdminEmailTemplate = getEmailTemplate($emailType, ADMIN_LANGUAGE, ADMIN_ROLES);

                        if ($getAdminEmailTemplate) {

                            $language = \Config\Services::language();
                            $language->setLocale(getLanguageCode(2, true));
                            ...
                            $message = view('emailTemplateView', ['message' => $content, 'disclaimerText' => getEmailDisclaimer(ADMIN_LANGUAGE), 'footer' => getEmailFooter(ADMIN_LANGUAGE)]);
                        ...
                        // create Activity log
                        $activityEvent = 'deleteMyAccountAdmin';
                        $replacements = [
                            // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                            '{USER_ID}' => '{USER_ID_' . $userId . '}',
                        ];
                        $additionalData = ['activity_type_id' => 4]; // created
                        logActivity($activityEvent, $replacements, $additionalData);
                    } else {
                        $response = [
                            "status"    => false,
    ...
    public function deleteMyAccount()
    {
        ...
        if (!$user) {
            ...
            if ($users->delete($user->id)) {
                ...
                $getAdminEmailTemplate = getEmailTemplate($adminEmailType, ADMIN_LANGUAGE, ADMIN_ROLES);
                ...
                // create Activity log
                $activityEvent = 'deleteMyAccount';
                $replacements = [
                    // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                    '{USER_ID}' => '{USER_ID_' . $user->id . '}',
                ];
                $additionalData = ['activity_type_id' => 4]; // created
                logActivity($activityEvent, $replacements, $additionalData);
                // Return a success message as a JSON response
    ...
    public function getUsers($lang = null)
    {
        ...
        $searchParams = getQueryString($url); 
        ...
        $users = getPlayers($whereClause, $metaQuery, $search, $orderBy, $order, $limit, $offset, $noLimit, $lang);
    ...
    public function getUsersOnFrontend()
    {
        ...
        $queryString =  $uri->getQuery();
        parse_str($queryString, $searchParams);
        // echo '>>>>>>>>>>>>>> getUsersOnFrontend whereClause >>>>>>>>>>>>>>';
        // pr($searchParams);
        ...
        $users = getPlayersOnFrontend($whereClause, $metaQuery, $search, $orderBy, $order, $limit, $offset, $noLimit, $currentLang);
                ...
                foreach ($ids as $id) {
                    ...
                    if ($user) {
                        ...
                        if ($builder->update()) {
                            ...
                            if ($getEmailTemplate) {
                                $this->request->setLocale(getLanguageCode($user->lang, true));

                                $replacements = [
                                    '{firstName}'           => $user->first_name,
                                    '{loginLink}'           => '<a href="' . getPageInUserLanguage($user->user_domain, $user->lang, "login") . '">' . lang('Email.login') . '</a>',
                                    '{PricingLink}'         => '<a href="' . getPageInUserLanguage($user->user_domain, $user->lang, 'pricing') . '">' . lang('Email.premiumMembership') . '</a>',
                                    '{helpEmail}'           => '<a href="mailto:' . HELP_EMAIL . '">' . lang('Email.helpEmail')  . '</a>',
                            ...
                            $activity_data = array();
                            $activityMsgArr = getActivityLogs(array('activity_event' => 'adminUpdatedUserStatus'));
                            $languages = ACTIVITY_LANG_SLUGS;
                            $activity_data = [
                                'user_id'               => $id,
                                'activity_type_id'      => 5, // login activity type
                                'activity'              => 'adminUpdatedUserStatus',
                                'ip'                    => $this->request->getIPAddress()
                            ];
                            if (isset($activityMsgArr) && !empty($activityMsgArr)) {
                                foreach ($languages as $lang) {
                                    ...
                                    if (isset($activityMsgArr['user_message_' . $lang])) {
                                        $msg =  $activityMsgArr['user_message_' . $lang];
                                        $msg = str_replace('{STATUS}', $status, $msg);
                                        $activity_data['activity_' . $lang] = $msg;
                                    }
                                    if (isset($activityMsgArr['admin_message_' . $lang])) {
                                        $msg =  $activityMsgArr['admin_message_' . $lang];
                                        $msg = str_replace('{STATUS}', $status, $msg);
                                        $msg = str_replace('{USER_ID}', '{USER_ID_' . $id . '}', $msg);
                                        $activity_data['admin_activity_' . $lang] = $msg;
                                    }
                                }
                            }
                            createActivityLog($activity_data);
                            ...
                            $response = [
                                "status"    => true,
                                "message"   => lang('App.userStatusUpdated', [], getLanguageCode($currentLang, true)),
                                "data"      => []
                            ];
                        } else {
                            $response = [
                                "status"    => false,
                                "message"   => lang('App.userStatusUpdateFailed', [], getLanguageCode($currentLang, true)),
                                "data"      => []
                            ];
                        }
                    }
    ...
    public function uploadProfileImage($lang = null)
    {
        ...
        $maxFileSizeKB = MAX_PROFILE_IMG_SIZE;
        ...
        $validationRule = [
            'profile_image' => [
                ...
                'rules' => [
                    ...
                    "max_size[profile_image, $maxFileSizeKB]",
                ...
                'errors' => [
                    ...
                    'max_size'  => lang('App.file_too_large', ['file' => $this->request->getFile('profile_image')->getClientName(), 'size' => "2 MB"]),
        ...
        if (! $this->validateData([], $validationRule)) {
            ...
            if ($is_uploaded && $is_uploaded['status'] == "success") {
                ...
                // create Activity log
                $activityEvent = 'uploadedProfileImage';
                $replacements = [
                    // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                    '{USER_ID}' => '{USER_ID_' . $userId . '}',
                ];
                ...
                $data = ['errors' => $is_uploaded, 'test' => 'sdfsdf'];
                $response = [
                    "status"    => false,
                    "message"   => lang('App.profile_updatedfailure'),
                    "data"      => $data
    ...
    public function uploadCoverImage($lang = null)
    {
        ...
        $maxFileSizeKB = MAX_COVER_IMG_SIZE;
        $validationRule = [
            'cover_image' => [
                'label' => 'Cover Image',
                'rules' => [
                    ...
                    "max_size[cover_image, $maxFileSizeKB]",
                ...
                'errors' => [
                    ...
                    'is_image'  => lang('App.is_image', ['file' => 'jpg, jpeg, gif, png, webp']),
                    'mime_in'   => lang('App.mime_in', ['file' => 'jpg, jpeg, gif, png, webp']),
                    'max_size'  => lang('App.file_too_large', ['file' => $this->request->getFile('cover_image')->getClientName(), 'size' => "2 MB"]),
                    'max_dims'  => lang('App.max_dims'),
                ],
            ],
        ];
        ...
        if (! $this->validateData([], $validationRule)) {
            ...
            if (! $cover_image->hasMoved()) {
                ...
                $activityEvent = 'uploadedCoverImage';
                $replacements = [
                    // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                    '{USER_ID}' => '{USER_ID_' . $userId . '}',
                ];
                ...
                $response = [
                    "status"    => true,
                    "message"   => lang('App.coverImageUploaded'),
                    "data"      => $data
                ];
            } else {
                ...
                $response = [
                    "status"    => false,
                    "message"   => lang('App.coverImageUploadFailure'),
                    "data"      => $data
                ];
            }
        }
    ...
    public function deleteProfileImage($lang = null)
    {
        ...
        if ($userData) {
            ...
            if ($res) {
                // $data = ['uploaded_fileinfo' => $cover_image->getName()];
                $activityEvent = 'deletedProfileImage';
                $replacements = [
                    // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                    '{USER_ID}' => '{USER_ID_' . $userData['id'] . '}',
                ];
                $additionalData = ['activity_type_id' => 4]; // created
                logActivity($activityEvent, $replacements, $additionalData);
    ...
    public function deleteCoverImage($lang = null)
    {
        ...
        if ($userData) {
            ...
            if ($res) {
                // create Activity log
                $activityEvent = 'deletedCoverImage';
                $replacements = [
                    // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                    '{USER_ID}' => '{USER_ID_' . $userId . '}',
                ];
                ...
                $response = [
                    "status"    => true,
                    "message"   => lang('App.coverImageDeleted'),
                    "data"      => []
                ];
            } else {
                $response = [
                    "status"    => false,
                    "message"   => lang('App.coverImageDeleteFailure'),
                    "data"      => []
                ];
            }
        } else {
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP)) {
            ...
            $rules = [
                ...
                "email" => [
                    ...
                    'errors' => [
                        'is_unique' => 'Validation.emailExist',
                    ]
                ],
                "site_role"     => "required",
                "first_name"    => "required",
            ...
            if (!$this->validate($rules)) {
                ...
                // if (auth()->user()->role == 2) {
                if (in_array(auth()->user()->role, [2, 6])) {
                    $group = 'club-representator';
                    $role = 6;
                }

                if (in_array(auth()->user()->role, [3, 7])) {
                ...
                if ($users->save($userData)) {
                    $representator_id = $users->getInsertID();
                    ...
                    $userInfo->addPermission($this->request->getVar("site_role"));         // add user permission  // admin.edit
                    ...
                    // ];
                    // createActivityLog($activity_data); 
                    // $representator_id adminAddRepresentator

                    // An admin has added you as a Representative.

                    // create Activity log

                    if (in_array(auth()->user()->role, ADMIN_ROLES)) {
                        $activityEvent = 'adminAddRepresentator';

                        $replacements = [
                            // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                            '{USER_ID}' => '{USER_ID_' . $representator_id . '}',
                        ];
                    } else {
                        $activityEvent = 'addedRepresentator';

                        $replacements = [
                            // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                            '{USER_ID}' => '{USER_ID_' . auth()->id() . '}',
                            '{REPRESENTATOR_ID}' => '{REPRESENTATOR_ID_' . $representator_id . '}',

                        ];
                    }
                    //addedRepresentator

                    // $additionalData = ['activity_type_id' => 4, 'user_id' => $representator_id]; // created
                    $additionalData = ['activity_type_id' => 4, 'user_id' => auth()->id()]; // created
                    logActivity($activityEvent, $replacements, $additionalData);
    ...
    public function updateRepresentator($representatorID = null)
    {
        $currentLang = $this->request->getVar("lang") ?? DEFAULT_LANGUAGE;
        $this->request->setLocale(getLanguageCode($currentLang, true));
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP) || hasAccess(auth()->user(), EDITOR_ACCESS)) {
            ...
            $user_id = $representatorID ?? auth()->id();
            // $adminId = auth()->id();
            ...
            if (!$this->validate($rules)) {
                ...
                $users = auth()->getProvider();
                $representatorInfo = $users->findById($representatorID);

                $representatorInfo->fill([
                    "first_name"    => ucfirst($this->request->getVar("first_name")),
                    "last_name"     => ucfirst($this->request->getVar("last_name")),
                ]);

                if ($users->save($representatorInfo)) {

                    if ($this->request->getVar("designation")) {

                        $save_data['user_id'] = $user_id;
                        $save_data['meta_key'] = 'designation';
                        $save_data['meta_value'] = $this->request->getVar("designation");
                        $userInfo = $userMetaObject->where('user_id', $user_id)->where('meta_key', 'designation')->first();

                        if ($userInfo) {
                            $save_data['id'] = $userInfo['id'];
                        }
                        $userMetaObject->save($save_data);
                    }
    ...
    public function updateRepresentatorRole($userId = null, $lang = null)
    {
                ...
                if ($representatorInfo->syncPermissions($this->request->getVar("site_role"))) {
                    ...
                    if ($this->request->getVar("site_role") == "transfer-ownership") {
                        ...
                        if ($authRole == "Super Admin") {
                            ...
                            $authInfo = getUserByID(userID: $representatorInfo->parent_id);
                            // pr($authInfo); exit;
                    ...
                    // create Activity log
                    $site_role = $this->request->getVar("site_role");
                    $roleInfo = getRepresentatorRoleBySlug($site_role);
                    $representator_id = $representatorInfo->id;

                    if (in_array(auth()->user()->role, ADMIN_ROLES)) {
                        $activityEvent = 'adminUpdateRepresentatorRole';
                    } else {
                        $activityEvent = 'updatedRepresentatorRole';
                    }

                    //updatedRepresentatorRole

                    $replacements = [
                        // '{USER_ROlE}'           => $site_role,
                        // '{USER_ID}'             => '{USER_ID_' . $representator_id . '}',
                        '{REPRESENTATOR_ROLE}'  => '{REPRESENTATOR_ROLE_' . $roleInfo->id . '}',
                        '{USER_ID}'             => '{USER_ID_' . auth()->id() . '}',
                        '{REPRESENTATOR_ID}'    => '{REPRESENTATOR_ID_' . $representator_id . '}',
                    ];
                    // $additionalData = ['activity_type_id' => 4, 'user_id' => $representator_id]; // created
    ...
    public function deleteRepresentator($user_id = null, $lang = null)
    {
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP)) {
            ...
            if ($del_res == true) {
                // create Activity log
                $activityEvent = 'adminDeleteRepresentatorRole';
                $replacements = [
                    '{USER_ID}' => '{USER_ID_' . $user_id . '}',
                ];
                $additionalData = ['activity_type_id' => 4, 'user_id' => $user_id]; // created
                logActivity($activityEvent, $replacements, $additionalData);
    ...
    public function userProfile($lang = null, $user_id = null)
    {
        $response = [];
        $currentLang = $lang ?? DEFAULT_LANGUAGE;
        // if (empty($this->request->getVar("lang"))) {
        ...
        if (auth()->id() && (in_array(auth()->user()->role, REPRESENTATORS_ROLES))) {
            if ($user_id) {
                ...
                $authUser = auth()->user();
                $id = $authUser->parent_id;
                $user_role = $authUser->role;
            }
        }

        // echo '>>>>>>>>>>>>> user_role >>> ' . $user_role; exit;
        ...
        if($user_role == 4){
            $builder->select('
                ...
                CONCAT("' . $imagePath . '", cb2.club_logo) as club_logo_path,
        ...
        if($user_role == 2){
            $builder->select('
                cb2.club_name as current_club_name,
                cb2.club_logo as club_logo,
                d3.location_' . $currentLangCode . ' as current_club_country,
                c3.country_flag as current_club_country_flag,
        ...
        // to get club details  of player/talent
        if($user_role == 4){
        ...
        if($user_role == 2){
            ...
            $builder->join('clubs cb2', 'cb2.id = um5.meta_value AND cb2.status = "active"', 'LEFT');
            // $builder->join('clubs cb2', 'cb2.taken_by = users.id AND cb2.status = "active"', 'LEFT');
            $builder->join('countries c3', 'c3.id = cb2.country_id', 'LEFT');
            $builder->join('domains d3', 'd3.country_id = cb2.country_id', 'LEFT');

            // to get club's league
            $builder->join('league_clubs lc', 'lc.club_id = cb2.id', 'LEFT');
            $builder->join('leagues l', 'l.id = lc.league_id', 'LEFT');
        }
        ...
        //  get Representetor data
        if (auth()->id() && (in_array(auth()->user()->role, REPRESENTATORS_ROLES))) {
            $repBuilder = $this->db->table('users');
            $repBuilder->select('users.*, 
                            user_roles.role_name as role_name,
                            languages.language as language,
                            domains.domain_name as user_domain,
                            domains.location as user_location,
            ...
            $repBuilder->join('auth_identities auth', 'auth.user_id = users.id AND type = "email_password"', 'LEFT');
            $repBuilder->join('user_roles', 'user_roles.id = users.role', 'LEFT');
            $repBuilder->join('languages', 'languages.id = users.lang', 'LEFT');
            $repBuilder->join('domains', 'domains.id = users.user_domain', 'LEFT');
            $repBuilder->join('auth_permissions_users apu', 'apu.user_id = users.id', 'LEFT');
            $repBuilder->where('users.id', auth()->id());
            $repData   = $repBuilder->get()->getRow();
        }
        ...
        if ($userData) {
            ...
            if (auth()->id() && (in_array(auth()->user()->role, REPRESENTATORS_ROLES))) {
                $response['data']['representator_data'] = $repData;
            }
        } else {
            // Invalid token
            $response = [
    ...
    public function updateGeneralInfoAdmin($id = null)
    {
        ...
        if ($res) {
            ...
            // create Activity log
            $activityEvent = 'userUpdatedProfileDetailAdmin';
            $replacements = [
                // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                '{USER_ID}' => '{USER_ID_' . $userId . '}',
            ];
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP)) {
            ...
            if (! $this->validateData([], $validationRule)) {
                ...
                if (! $cover_image->hasMoved()) {
                    ...
                    // create Activity log
                    $activityEvent = 'uploadedCoverImageAdmin';
                    $replacements = [
                        // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                        '{USER_ID}' => '{USER_ID_' . $userId . '}',
                    ];
                    // $additionalData = ['activity_type_id' => 4]; // created
                    $additionalData = ['activity_type_id' => 4, 'user_id' => $userId];
                    logActivity($activityEvent, $replacements, $additionalData);
                    $response = [
                        "status"    => true,
                        "message"   => lang('App.profile_updatedSuccess'),
    ...
    public function deleteCoverImageAdmin($userId = null)
    {
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP)) {
            ...
            if ($userData) {
                ...
                if ($res) {
                    // create Activity log
                    $activityEvent = 'deletedCoverImageAdmin';
                    $replacements = [
                        // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                        '{USER_ID}' => '{USER_ID_' . $userId . '}',
                    ];
                    $additionalData = ['activity_type_id' => 4, 'user_id' => $userId]; // created
                    logActivity($activityEvent, $replacements, $additionalData);
                    $response = [
                        "status"    => true,
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP)) {
            ...
            if (! $this->validateData([], $validationRule)) {
                ...
                if ($is_uploaded && $is_uploaded['status'] == "success") {
                    ...
                    // create Activity log
                    $activityEvent = 'uploadedProfileImageAdmin';
                    $replacements = [
                        // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                        '{USER_ID}' => '{USER_ID_' . $userId . '}',
                    ];
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP)) {
            ...
            if ($userData) {
                ...
                if ($res) {
                    ...
                    // create Activity log
                    $activityEvent = 'deletedProfileImageAdmin';
                    $replacements = [
                        // '{POPUP_ID}' => '{POPUP_ID_'.$id.'}',
                        '{USER_ID}' => '{USER_ID_' . $userId . '}',
                    ];
            ...
            if (!$this->validate($rules)) {
                ...
                if (!$clubHistory) {
                    ...
                    if ($userMetaObject->save($save_data)) {
                        ...
                        foreach ($languages as $lang) {
                            $languageService->setLocale($lang, true);
    ...
    public function updateClubHistory($id = null)
    {
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP) || hasAccess(auth()->user(), EDITOR_ACCESS)) {
            ...
            if (!$this->validate($rules)) {
                ...
                } else if (!$clubHistory && empty($clubHistory['id'])) {
                    $save_data = [
                        'user_id'            => $userID,
                        'meta_key'    => 'club_history',
                        'meta_value'    => $this->request->getVar("club_history"),
    ...
    public function addCompanyHistory($id = null)
    {
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP)) {
            ...
            if (!$this->validate($rules)) {
                ...
                $companyHistory = $userMetaObject->where('user_id', $userID)->where('meta_key', 'company_history')->first();
                // echo '<pre>'; print_r($companyHistory); die;
                $save_data = [
                    'user_id'       => $userID,
                ...
                } else if (isset($companyHistory) && !empty($companyHistory['id'])) {
                    $save_data['id'] = $companyHistory['id'];
                    $userMetaObject->updateMetaData($save_data);
                    $activity_data = array();
                    $languageService = \Config\Services::language();
                    $languages = ['en', 'de', 'fr', 'it', 'es', 'pt', 'da', 'sv'];
                    $activity_data = [
    ...
    public function editCompanyHistory($id = null)
    {
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP) || hasAccess(auth()->user(), EDITOR_ACCESS)) {
            ...
            if (!$this->validate($rules)) {
                ...
                $companyHistory = $userMetaObject->where('user_id', $userID)->where('meta_key', 'company_history')->first();
                if(empty($companyHistory) && !isset($companyHistory['id'])){
                    $save_data = [
                        'user_id'            => $userID,
                        'meta_key'      => 'company_history',
                        'meta_value'    => $this->request->getVar("company_history"),
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP) || hasAccess(auth()->user(), EDITOR_ACCESS)) {
            ...
            if (!$this->validate($rules)) {
                ...
                if ($this->request->getVar("newsletter") == 1) {
                    ...
                    $activity_event = 'newsletterSubscribed';
                } else {
                    $newsletterStatus = 'unsubscribed';
                    $activity_event = 'newsletterUnsubscribed';
                }
                ...
                if ($builder->update()) {
                    ...
                    $response = [
                        "status"    => true,
                        "message"   => lang('App.' . $activity_event),
    ...
    public function getActivity()
    {
        ...
        if (!empty($lang) && is_numeric($lang)) {
            ...
            // $userlocale = getLanguageCode($lang, true);
            $userlang = getLanguageCode($lang);
        }
        ...
        if ($userId == 1) {
            ...
            // $activities1 = $ActivityModel->getActivities($userId, $limit, $offset, $userlang);
            // $totalCount = $ActivityModel->getActivityCount($userId, $search);
            // $totalCount = count((array)$activities);
        }
        if ($activities) {
            $response = [
                'status' => true,
    ...
    public function deleteActivity()
    {
        ...
        if (is_array($ids) && count($ids) > 1) {
            ...
            // $data = ['message' => lang('App.activityDeletedSuccess')];
            $response = [
                "status" => true,
                "message" => lang('App.activityDeletedSuccess')
                // "data"   => $data
            ];
        } else {
            ...
            if (!empty($ids) && count($ids) == 1) {
                ...
                // $data = ['errors' => lang('App.activityDeleteFailed')];
                $response = [
                    "status" => false,
                    "message"   => lang('App.activityDeleteFailed')
                ];
            }
        }
        return $this->respondCreated($response);
    ...
    public function downloadCsv()
    {
        ...
        // }
        // $this->request->setLocale(getLanguageCode($currentLang, true));;
        // echo $currentLang; die(' $currentLang');
        $url =  isset($_SERVER['HTTPS']) &&
            $_SERVER['HTTPS'] === 'on' ? "https://" : "http://";
        $url .= $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
        $parse_url = parse_url($url);
        ...
        if (!empty($currentLang) && $currentLang == 2) {
    ...
    public function downloadUserPdf($userId = null, $lang = null)
    {
        // echo auth()->id(); die;
        set_time_limit(120);
        $currentLang = $lang ?? DEFAULT_LANGUAGE;
        ...
        if (!empty($userId)) {
            ...
            * {box-sizing: border-box;}
            ...
            .gallery-imgs{   box-sizing: border-box;height: 200px;}
            </style>
            </head>';
            ...
            if (isset($userData['role']) && strtoupper($userData['role']) == '4') { // Player
                ...
                $content .= ' <img src="' . $profile_image_path . '" style="width:270px; max-height: 180px;">';
                $content .= '</td>';
                $content .= '</tr>';
                $content .= '  </table>';
                $content .= ' </td>';
                ...
                $in_team_since = '';
                if (!empty($userData['meta']['in_team_since']) && trim($userData['meta']['in_team_since']) != 'Invalid date' && trim($userData['meta']['in_team_since']) != 'undefined') {

                    $in_team_since = $userData['meta']['in_team_since'];
                    // echo $in_team_since; die;
                ...
                $date_of_birth = '';
                if (isset($userData['meta']['date_of_birth']) && !empty($userData['meta']['date_of_birth']) && trim($userData['meta']['date_of_birth']) != 'undefined') {
                ...
                if (!empty($userData['meta']['last_change']) && trim($userData['meta']['last_change']) != 'Invalid date' && trim($userData['meta']['last_change']) != 'undefined') {
                ...
                $contract = '';
                if (!empty($userData['meta']['contract_start']) && !empty($userData['meta']['contract_end'])  && trim($userData['meta']['contract_end']) != 'undefined') {
                ...
                if (!empty($transferDetail)) {
                    ...
                    foreach ($transferDetail as $transfer) {
                        ...
                        if (isset($transfer['date_of_transfer']) && !empty($transfer['date_of_transfer']) && trim($transfer['date_of_transfer']) != 'Invalid date'  && trim($transfer['date_of_transfer']) != 'undefined') {
                            // if (isset($transfer['date_of_transfer']) && !empty($transfer['date_of_transfer']) && trim($transfer['date_of_transfer']) != 'Invalid date'  && trim($userData['meta']['date_of_transfer']) != 'undefined') {
            ...
            if (isset($userData['role']) && strtoupper($userData['role']) == '3') { // SCOUT 
                ...
                if (!empty($role_name)) {
                    ...
                    $content .= '<td>';
                    // $content .= '<img src="' . $flagPath . 'logo.png" style="width: 150px;vertical-align: top;margin-top: -4px;">';
                    $content .= '<img src="' . $flagPath . 'Newlogo.png" style="width: 150px;vertical-align: top;margin-top: -4px;">';
                    $content .= '<span style="font-size: 20px;padding-left: 5px;">' . strtoupper($role_name) . ' PROFILE</span>';
                    $content .= '</td>';
                    $content .= '</tr>';
                }
            ...
            try {
                ...
                if (auth()->id() == 1) {
                    $activity_key = 'downloadUserPdfAdmin';
                } else {
                    $activity_key = 'downloadUserPdf';
                }
            ...
            if (file_exists($userPdfPath)) {
                ...
                $response = [
                    "status"    => true,
                    "message"   => lang('App.fileCreatedSuccess', [], getLanguageCode($currentLang, true)), 
                    "data"      => [
                        "file_name" => $filename,
                        "file_path" => base_url() . $folder_nd_file
                    ]
                ...
                $response = [
                    "status"    => false,
                    "message"   => lang('App.noDataFound', [], getLanguageCode($currentLang, true)), //lang('App.pdfDownloadFailed'), // 
                    "data"      => ['error' => $errors]
                ];
            }
        } else {
            $response = [
                "status"    => false,
                "message"   => lang('App.invalid_userID', [], getLanguageCode($currentLang, true)), //lang('App.pdfDownloadFailed'), // 
                "data"      => []
            ];
        }
        ...
        if ($search) {
            ...
            if (auth()->id()) {
                ...
                if (!in_array(auth()->id(), ADMIN_ROLES)) {
                    $builder->where("u.status", 2);     // 2 => verified users
                }
            }
            ...
            // $builder->orLike("CONCAT(u.first_name, ' ', u.last_name)", $search, false);
            $builder->orLike('CONCAT(TRIM(u.first_name), " ", TRIM(u.last_name))', $search);
            $builder->groupEnd();
            ...
            $query = $builder->get();
            // echo ' >>>>>>>> searchBar query >>>>>>>>>> ' . $this->db->getLastQuery();
